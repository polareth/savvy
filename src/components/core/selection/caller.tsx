...
import { callContract } from '@/lib/tevm/call';
...
const CallerSelection = () => {
  ...
  const { account, abi, caller, isHydrated, setCaller, resetCaller } =
    useConfigStore((state) => ({
      ...
      // Reset the current address to impersonate as the caller (back to DEFAULTS.caller)
  ...
  return (
    <div className="mt-2 flex flex-col gap-2">
      <div className="flex w-full items-center gap-4">
        <span className="flex grow items-center text-sm font-medium text-muted-foreground">
          <Icons.bullet /> Caller
        </span>
        <TooltipResponsive
          content={
      ...
      <div className="flex flex-col items-stretch gap-2 md:gap-2">
        <div className="flex items-center gap-2">
          <Input
            id="caller"
            type="text"
            className={cn(
              'font-mono text-xs md:min-w-[250px] md:text-sm',
          ...
          {isDesktop ? null : (
            <Button
              size="sm"
              variant="ghost"
              className="flex w-min items-center gap-2 text-muted-foreground"
        ...
        {isHydrated && !fetchingOwner ? (
          isDesktop ? (
            <TooltipResponsive
              trigger={
                <span className="flex items-baseline gap-2 text-sm font-medium text-muted-foreground opacity-80">
                  Currently set to{' '}
                  <ShrinkedAddress
                    address={caller}
