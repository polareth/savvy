...
export const LinkTo = ({
  ...
  <Link
    ...
    className="underline-offset-3 font-medium underline transition-colors duration-100 hover:text-muted-foreground"
  >
    {children}
  </Link>
);
  ...
  <ul className="ml-4 list-none [&:not(:first-child)]:mt-2 [&:not(:last-child)]:mb-2 [&>li]:mt-1">
    {items.map((item, i) => (
      <li key={i} className="flex items-start gap-2">
        <Icons.right className="mt-[3px] h-4 w-4 text-muted-foreground" />
        <span>{item}</span>
      </li>
    ))}
  </ul>
