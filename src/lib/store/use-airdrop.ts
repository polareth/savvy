...
export const useAirdropStore = create<SelectionAirdrop>((set, get) => ({
  ...
  toggleCustomToken: () =>
    set((state) => ({ customToken: !state.customToken })),
  setCustomTokenAddress: (customTokenAddress) => set({ customTokenAddress }),
  setcustomTokenOwnerOrHolder: (customTokenOwnerOrHolder) =>
    set({ customTokenOwnerOrHolder }),
  ...
  getCurrent: () => {
    ...
    return {
      ...
      solution:
        tokenId && methodId ? AIRDROP_SOLUTIONS[tokenId][methodId] : null,
    };
  },
  getSolution: () => {
    const { tokenOption, methodOption } = get();
